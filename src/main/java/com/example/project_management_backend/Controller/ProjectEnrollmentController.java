...
public class ProjectEnrollmentController {
    ...
    public ResponseEntity<ApiResponse<List<User>>> getApprovedStudents(@RequestParam UUID projectId) {
        List<User> students = enrollmentService.getApprovedStudentsForProject(projectId);
        return ResponseEntity.ok(new ApiResponse<>(HttpStatus.OK.value(), "Approved students fetched successfully", students));
    }
