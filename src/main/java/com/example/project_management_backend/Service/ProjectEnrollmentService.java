...
public class ProjectEnrollmentService {
    ...
    public List<User> getApprovedStudentsForProject(UUID projectId) {
        List<ProjectEnrollment> enrollments = enrollmentRepository.findByProject_ProjectIdAndStatusAndDeletedAtIsNull(
                projectId, "APPROVED");

        return enrollments.stream().map(ProjectEnrollment::getStudent).collect(Collectors.toList());
    }
